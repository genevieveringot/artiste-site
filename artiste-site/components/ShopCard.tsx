...
export function ShopCard({ painting, index }: ShopCardProps) {
  const [isModalOpen, setIsModalOpen] = useState(false)
  
  // Simulate original price (20% higher)
  const originalPrice = painting.price ? Math.round(painting.price * 1.2) : null
  const hasDiscount = originalPrice !== null && painting.price !== null
          ...
          >
            ...
            <div className="flex flex-col justify-center py-8">
              ...
              <div className="flex items-center gap-4 mb-8">
                {hasDiscount && (
                  <span className="text-[#555] line-through text-lg">
                    {formatPrice(originalPrice)}
                  </span>
                )}
                <span className="text-[#c9a86c] text-2xl">
                  {formatPrice(painting.price)}
                </span>
              </div>
